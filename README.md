...
disponíveis: [Flyway Gradle Plugin](https://flywaydb.org/documentation/usage/gradle/info)
...
Para rodar a aplicação localmente com Docker, iremos utilizar o `docker compose` e necessita de
apenas três passos:
<br/>
...
Depois de visualizar que os demais containers estão de pé, para rodar sua aplicação junto basta
executar o comando:
...
- Realm Settings -> Security Defenses -> Content-Security-Policy:
  `frame-src 'self'; frame-ancestors 'self' http://localhost:3000;`
